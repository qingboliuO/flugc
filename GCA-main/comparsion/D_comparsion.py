...
if __name__ == '__main__':#比较了
    ...
    # print(device)
    # 加载数据
    # data = pd.read_csv('data/train/Liao_H3N2.csv')
    # data = pd.read_csv('data/train/Yuhua_Yao_H3N2.csv')#有空值要值1
    data = pd.read_csv('data/train/Lees_H3N2_new_epitope_data.csv')
    # data = pd.read_csv('data/train/Min-Shi Lee_H3N2.csv')#没有Unnamed: 0这一列
    ...
    # print(X_train.head(2).to_string(index=False, header=False))
    #
    # # 打印对应的训练集标签的前两行
    # print("First two rows of y_train:")
    # print(y_train.head(2).to_string(index=False, header=False))
    # 数据标准化
    ...
    # avg_mae = np.mean([result[1] for result in results])
    # avg_r2 = np.mean([result[2] for result in results])
    # # 打印每折结果
    # for i, (mse, mae, r2) in enumerate(results, start=1):
    #     print(f"Fold {i}")
