...
H1N1_seq = pd.read_excel('../data/jiaH1N1_antigenicDistance.xlsx')
H1N1_Antigenic_dist = pd.read_excel('../data/jiaH1N1_antigenicDistance.xlsx')
H3N2_seq = pd.read_excel('../data/jiaH3N2_antigenicDistance.xlsx')
H3N2_Antigenic_dist = pd.read_excel('../data/jiaH3N2_antigenicDistance.xlsx')
# H5N1_seq = pd.read_csv('../data/jiaH5N1_antigenicDistance.csv', names=['seq', 'description'])
# H5N1_Antigenic_dist = pd.read_csv('../data/jiaH5N1_antigenicDistance.csv')
##################################################################################################
...
def Liao_feature_engineering(distance_input, seq_input, subtype):  # distance_input, seq_input都是dataframe数据
    ...
    print(seq_input.columns)
    # 蛋白质序列的长度
    length = len(seq_input['seq1'].iloc[0])
    ...
    for i in range(0, distance_input.shape[0]):
        ...
        for j in range(0, seq_input.shape[0]):
            ...
            strain_1 = seq_input['seq1'].iloc[j].upper()#给当前毒株赋值它的序列
            # if seq_input['description'].iloc[j].upper() == distance_input['strain2'].iloc[i].upper():
            strain_2 = seq_input['seq2'].iloc[j].upper()
...
# Liao_feature_H1N1.to_csv('C:\Users\刘青博\PycharmProjects\pythonProject17\GCA-main\comparsion\data\train/Liao_H1N1.csv')
...
# Liao_feature_H5N1.to_csv('C:\Users\刘青博\PycharmProjects\pythonProject17\GCA-main\comparsion\data\train/Liao_H1N1.csv')
...
def Yao_feature_engineering(distance_input, seq_input, subtype):
    ...
    if subtype == 0:
        ...
        columns = list(range(1, 330, 1))#329
    elif subtype == 2:
        columns = list(range(1, 321, 1))
    for col in range(len(columns)):
        columns[col] = str(columns[col])
    ...
    for i in range(0, distance_input.shape[0]):
        ...
        strain_2 = []
        for j in range(0, seq_input.shape[0]):
            # if seq_input['seq1'].iloc[j].upper() == distance_input['strain1'].iloc[i].upper():
            strain_1 = seq_input['seq1'].iloc[j].upper()
            # if seq_input['description'].iloc[j].upper() == distance_input['strain2'].iloc[i].upper():
            strain_2 = seq_input['seq2'].iloc[j].upper()
        for a in range(0, length):
...
def distance_mutation(distance_input, seq_input):
    ...
    for i in range(0, distance_input.shape[0]):
        ...
        for j in range(0, seq_input.shape[0]):
            ...
            strain_1 = seq_input['seq1'].iloc[j]
            # if seq_input['description'].iloc[j] == distance_input['strain2'].iloc[i]:
            strain_2 = seq_input['seq2'].iloc[j]
        mutation_count = sum(0 if c1 == c2 else 1 for c1, c2 in zip(strain_1, strain_2))
        num_mut_list.append(mutation_count)
    return num_mut_list


def distance_mutation(distance_input):
    # 该函数接收包含序列信息的 DataFrame，计算突变数，并返回突变数列表
    num_mut_list = []
    for index, row in distance_input.iterrows():
        strain_1 = row['seq1']
        strain_2 = row['seq2']
        mutation_count = sum(1 for c1, c2 in zip(strain_1, strain_2) if c1 != c2)
        num_mut_list.append(mutation_count)
    return num_mut_list
...
# 计算突变数
mutation_counts = distance_mutation(H3N2_Antigenic_dist)
results_df = pd.DataFrame({
        'mutation_count': mutation_counts,
        'distance': H3N2_Antigenic_dist['distance']
    })
output_file = 'data/train/Min-Shi Lee_H3N2.csv'
...
def William_Lees(distance_input, seq_input):
    ...
    def generate_feature(region_type, distance_input, seq_input):
        ...
        for region in region_type:#一个区域一个区域遍历
            ...
            for i in range(0, distance_input.shape[0]):
                ...
                for a in range(0, len(site)):
                    ...
                    for j in range(0, seq_input.shape[0]):
                        # if seq_input['seq1'].iloc[j].upper() == distance_input['strain1'].iloc[i].upper():
                        value_1 = seq_input['seq1'].iloc[j][#给这个毒株给、当前区域的特定位点赋值序列
                            site[a] - 1]  # index is 0 by default, but the site starts from 1
                        # if seq_input['seq2'].iloc[j].upper() == distance_input['strain2'].iloc[i].upper():
                        value_2 = seq_input['seq2'].iloc[j][site[a] - 1]
                    if value_1 == value_2:
...
distance_path = "../data/jiaH3N2_antigenicDistance.xlsx"
distance_input = pd.read_excel(distance_path)
print(distance_input.columns)  # 查看所有列名
sequence_path = "../data/jiaH3N2_antigenicDistance.xlsx"
seq_input = pd.read_excel(sequence_path)
print('111')
# William_Lees_feature_H3N2 = William_Lees(distance_input, seq_input)
# Yao_feature_H3N2 = Yao_feature_engineering(H3N2_Antigenic_dist, H3N2_seq, 1)
# William_Lees_feature_H3N2.to_csv('data/train/William_Lees_H3N2.csv')
print("William_Lees")
