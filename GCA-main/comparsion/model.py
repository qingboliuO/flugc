...
#把每一对氨基酸的相似性从这个矩阵中提取出来，比如AA1.0 AC0.8 AD0.5
...
def Yao_feature_engineering(distance_input, seq_input, subtype):
    ...
    length = len(seq_input['seq'].iloc[0])
    if subtype == 0:
        columns = list(range(1, 328, 1))#创建Mut_feature列表，设置列名范围为1到328，最后一个1是步长，用来存储两对毒株每个位置的相似性
    elif subtype == 1:
        columns = list(range(1, 330, 1))
    elif subtype == 2:
        columns = list(range(1, 321, 1))
    ...
    for i in range(0, distance_input.shape[0]):#这里循环的是抗原距离对数
        strain_1 = []
        strain_2 = []
        for j in range(0, seq_input.shape[0]):
            if seq_input['description'].iloc[j].upper() == distance_input['Strain1'].iloc[i].upper():
                strain_1 = seq_input['seq'].iloc[j].upper()   #将这一对毒株的Strain1毒株序列赋值给strain_1
            if seq_input['description'].iloc[j].upper() == distance_input['Strain2'].iloc[i].upper():
                strain_2 = seq_input['seq'].iloc[j].upper()   #将这一对毒株的Strain2毒株序列赋值给strain_2
        for a in range(0, length):#遍历每个位置，aa_pair_1存储正对，aa_pair_2存储反对例如AC CA
            ...
            for b in range(0, Yao_embedding_table.shape[0]):
                if strain_1[a] == '-' or strain_2[a] == '-':
                    Mut_feature.iloc[i][a] = 0#如果该位置氨基酸有一个不确定就把相似度定义为0
                    break
                    ...
                    #i代表的是哪一对，a代表的是这一对哪个位置，然后把这个位置的氨基酸对相似度赋值给它
                    break

    Mut_feature = Mut_feature.join(label)      # 将标签加入特征矩阵
    return (Mut_feature)

#H1N1_seq = replace_uncertain_amino_acids(H1N1_seq)    #先替换未知氨基酸为已知的
...
def distance_mutation(distance_input, seq_input):#这个函数计算两毒株间的突变数。每当同一个位置上的氨基酸不同，就增加一个突变计数。
    num_mut_list = []
    for i in range(0, distance_input.shape[0]):
        mutation_count = 0
        strain_1 = []
...
def William_Lees(distance_input, seq_input):
    ...
    def generate_feature(region_type, distance_input, seq_input):
        distance_label = calculate_label(distance_input)
        label = {'label': distance_label}#提取出来每对毒株的抗原距离
        label = pd.DataFrame(label)
        ...
        for region in region_type:# 遍历每个区域
            site = region_type[region]# 获取当前区域的位点
            for i in range(0, distance_input.shape[0]):# 遍历每对毒株
                mut_count = 0
                for a in range(0, len(site)):# 遍历区域内的每个位点
                    value_1 = 0
                    value_2 = 0
                    for j in range(0, seq_input.shape[0]):
                        if seq_input['description'].iloc[j].upper() == distance_input['Strain1'].iloc[i].upper():
                            value_1 = seq_input['seq'].iloc[j][  # 获取第一个毒株在该位点的氨基酸
                                site[a] - 1]  # index is 0 by default, but the site starts from 1
                        if seq_input['description'].iloc[j].upper() == distance_input['Strain2'].iloc[i].upper():
                            value_2 = seq_input['seq'].iloc[j][site[a] - 1]# 获取第二个毒株在该位点的氨基酸
                    if value_1 == value_2:#该位点两个毒株的氨基酸相同则代表未突变
                        mut_count = mut_count + 0
                    else:
                        mut_count = mut_count + 1#该位点两个毒株的氨基酸不同则代表突变

                Mut_feature[region].iloc[i] = mut_count# 将突变数存入特征矩阵
        Mut_feature = Mut_feature.join(label)# 将标签加入特征矩阵
        return (Mut_feature)
    #只有划分的这些位点的氨基酸对抗原距离有影响，然后计算每个区域内所有位点氨基酸的突变数，为什么要划分区域是因为不同区域对抗原距离的影响权重不一样
